...
export default class GameEngine {
  ...
  async start() {
    ...
    if (userGoesFirst) {
      ...
      computerDiceIndex = this.computer.selectDice(this.diceList);
      this.cli.print(
        `I make the first move and choose the ${this.computer.dice.toString()} dice.`
      );
  ...
  checkWinner(computerInst, userInst) {
    ...
    const winStr =
      computer > user ? `${computer} > ${user}` : `${user} > ${computer}`;
  ...
  async makeTurn(range, isUser = false) {
    ...
    const final = isUser
      ? this.user.finalizeRoll(computerNumber)
      : this.computer.finalizeRoll(moduleIndex);
    this.cli.print(
      `The fair number generation result is ${computerNumber} + ${moduleIndex} = ${final.module} (mod 6).`
    );
    this.cli.print(
      isUser
        ? `Your roll result is ${final.value}.`
        : `My roll result is ${final.value}.`
    );
  }
}
